        ...
        public static void Exec() => new Thread(
            () =>
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    double proc = ((double) lineNum / ((double) lines.Length) * 100.0);

                    if ((int) proc != (int) prevProc)
                    {
                        Program.Form.chart.Invoke((MethodInvoker) (() => Program.Form.mainProgressBar.Value = (int) proc));
                        prevProc = proc;
                    }
